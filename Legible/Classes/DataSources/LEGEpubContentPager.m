...
- (LEGEpubChapter *)chapterAtIndex:(NSInteger)chapterIndex{
    return [self.chapterArray objectAtIndex:chapterIndex];
}
...
-(NSAttributedString *)attributedStringForAllText{
    if (self.allTextAttributedString) {
        return self.allTextAttributedString;
    }
    
    NSMutableAttributedString * attributedStringAllText = [NSMutableAttributedString new];
    for (NSString * spineKey in self.epubController.contentModel.spine) {
        @autoreleasepool {
            NSString *contentFile = self.epubController.contentModel.manifest[spineKey][@"href"];
            NSURL *contentURL = [self.epubController.epubContentBaseURL URLByAppendingPathComponent:contentFile];
            ...
            [attributedStringAllText appendAttributedString:attributedString];
        }
    }
    return attributedStringAllText;
}
